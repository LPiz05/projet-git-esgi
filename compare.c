...
int compare(struct Tag *tag, struct DtdTag *dtdTag)
{
    ...
    if (!checkTagName(tag->start, tag->end, dtdTag->tagName))
        return 0;
    if (!checkAttributs(tag->attributs, tag->attributsAmount, dtdTag->attributs, dtdTag->attributsAmount))
        return 0;
    ...
    for (int i = 0; i < tag->childsAmount; ++i)
    {
        ...
        for (int j = 0; j < dtdTag->childsAmount; ++j)
        {
            ...
            if (checkTagName(tag->childTags[i]->start, tag->childTags[i]->end,
                             dtdTag->childTags[j]->tagName))
            { //find a corresponding dtd for each child
                find = 1;
                ...
                { //si il a des enfants on les checks car c'est une balise parent
                    if (!compareChilds(tagToCheck, dtdToCheck))
                        return 0;
                }
                else
                { //pas d'enfants donc on check son content car c'est une balise enfant
                    if (!checkContent(tagToCheck->content, dtdToCheck->contentType))
                        return 0;
                }
            }
        }
...
{
    if (strstr(start, tagName) == NULL)
        return 1; //check in both way
    if (strstr(tagName, start) == NULL)
        return 0; //check in both way
    if (strstr(end, tagName) == NULL)
        return 0; //check in both way
    if (strstr(tagName, end) == NULL)
        return 0; //check in both way
    ...
    for (int i = 0; i < sizeAttr; ++i)
    {
        ...
        for (int j = 0; j < sizeDtdAttr; ++j)
        {
            if (checkAttrName(attrList[i]->name, dtdAttrList[j]->name))
            {
                ...
                if (!checkAttrValue(attrList[i]->value, dtdAttrList[i]))
                    return 0;
            }
        }
        if (!find)
            return 0; //pas trouvé d'occurrence
    }
...
{
    if (strstr(attrName, dtdAttrName) == NULL)
        return 0; //check in both way
    if (strstr(dtdAttrName, attrName) == NULL)
        return 0; //check in both way
    return 1;
}
...
int checkAttrValue(char *value, struct DtdAttr *dtdAttr)
{
    ...
    else
    {
        ...
        {
            if (!checkAttrValues(value, dtdAttr->values, dtdAttr->valuesSize))
                return 0;
        }
        else
        {
            if (!checkAttrType(value, dtdAttr->type))
                return 0;
        }
    }
    return 1;
...
int checkAttrValues(char *value, char **values, int valuesSize)
{
    ...
    {
        if (strstr(value, values[i]) && strstr(values[i], value))
            return 1;
    }
